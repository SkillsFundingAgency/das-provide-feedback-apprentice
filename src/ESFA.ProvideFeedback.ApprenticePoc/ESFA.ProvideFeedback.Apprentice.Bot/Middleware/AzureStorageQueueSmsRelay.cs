namespace ESFA.ProvideFeedback.Apprentice.Bot.Middleware
{
    ...
    using AzureConfiguration = Config.Azure;
    using ConnectionStrings = Config.ConnectionStrings;
    using DataConfiguration = Config.Data;
    using NotifyConfiguration = Config.Notify;

    /// <inheritdoc />
    /// <summary>
    /// Adds middleware to write conversation responses to a Queue for the purposes of using Notify to relay the message.
    /// </summary>
    public class AzureStorageQueueSmsRelay : ISmsRelayMiddleware
    {
        /// <summary>
        ...
        public async Task RelayMessage(ITurnContext context, Activity activity)
        {
